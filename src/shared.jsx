...
export const THEMES = {
  dark: {
    ...
    "--divider-gradient": "linear-gradient(90deg, transparent, rgba(200,169,110,0.4), transparent)",
    "--eyebrow-line": "rgba(200,169,110,0.5)",
    "--scroll-line": "linear-gradient(to bottom, rgba(200,169,110,0), rgba(200,169,110,1))",
  },
  light: {
    "--bg-solid": "#F5F0E8",
    ...
    "--line-rgb": "120,88,42",
    "--divider-gradient": "linear-gradient(90deg, transparent, rgba(139,92,30,0.4), transparent)",
    "--eyebrow-line": "rgba(139,92,30,0.45)",
    "--scroll-line": "linear-gradient(to bottom, rgba(139,92,30,0), rgba(139,92,30,0.8))",
  },
};
...
export const GLOBAL_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes widthExpand { from { width: 0; } to { width: 2.5rem; } }
  ...
  .slide-up      { animation: fadeSlideUp 0.7s cubic-bezier(0.16,1,0.3,1) both; }
`;
...
/* ─────────────────────────────────────────────────────────────────
   SHARED COMPONENTS
───────────────────────────────────────────────────────────────── */
export function ParticleCanvas({ particleRgb, lineRgb }) {
  const canvasRef = useRef(null);
  const colorsRef = useRef({ particleRgb, lineRgb });
  useEffect(() => { colorsRef.current = { particleRgb, lineRgb }; }, [particleRgb, lineRgb]);
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() { ctx.beginPath(); ctx.arc(this.x, this.y, this.r, 0, Math.PI * 2); ctx.fillStyle = `rgba(${colorsRef.current.particleRgb}, ${this.alpha})`; ctx.fill(); }
    }
    resize();
    ...
    function drawLines() {
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 130) { const a = (1 - dist / 130) * 0.12; ctx.beginPath(); ctx.moveTo(particles[i].x, particles[i].y); ctx.lineTo(particles[j].x, particles[j].y); ctx.strokeStyle = `rgba(${colorsRef.current.lineRgb}, ${a})`; ctx.lineWidth = 0.5; ctx.stroke(); }
        }
      }
    }
    function loop() { ctx.clearRect(0, 0, W, H); particles.forEach(p => { p.update(); p.draw(); }); drawLines(); animId = requestAnimationFrame(loop); }
...
export function ThemeToggle({ theme, onToggle }) {
  ...
  return (
    <button onClick={onToggle} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      style={{
        position: "fixed", top: "1.5rem", right: "1.75rem", zIndex: 100,
...
export function Logo({ theme, height = 36, style = {} }) {
  const src = theme === "dark" ? "/img/logo-dark.png" : "/img/logo-light.png";
  return (
    <img src={src} alt="Fermionisme" height={height} width={height}
      style={{ height: `${height}px`, width: "auto", display: "block", objectFit: "contain", transition: "opacity 0.3s ease", ...style }}
    />
  );
}

/* ─────────────────────────────────────────────────────────────────
   PAGE NAV BAR — logo left, back link, theme toggle right
───────────────────────────────────────────────────────────────── */
export function PageNav({ theme, onToggle, backLabel = "Back", backHref = "/" }) {
  ...
  return (
    <>
      ...
      <a
        href="#"
        onClick={(e) => { e.preventDefault(); navigate(backHref); }}
        onMouseEnter={() => setBackHovered(true)}
        onMouseLeave={() => setBackHovered(false)}
        style={{
          position: "fixed", top: "1.5rem", left: "1.75rem", zIndex: 100,
          display: "flex", alignItems: "center", gap: "0.5rem",
          fontFamily: "'Montserrat', sans-serif", fontSize: "0.7rem",
          fontWeight: 500, letterSpacing: "0.18em", textTransform: "uppercase",
          textDecoration: "none",
          color: backHovered ? "var(--accent)" : "var(--text-muted)",
          transition: "color 0.25s ease",
        }}
      >
        <svg width="14" height="10" viewBox="0 0 14 10" fill="none">
          <path d="M13 5H1M5 1L1 5l4 4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        {backLabel}
      </a>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.85rem" }}>
        ...
        <div>
          <div style={{
            ...
            letterSpacing: "0.1em", color: "var(--text-secondary)", lineHeight: 1, marginBottom: "0.2rem",
          }}>Fermionisme</div>
          <div style={{
            fontFamily: "'Montserrat', sans-serif", fontSize: "0.68rem",
            fontWeight: 300, letterSpacing: "0.18em", textTransform: "uppercase",
            color: "var(--accent-muted)", opacity: 0.6,
          }}>Fermion is me.</div>
        </div>
      </div>
        ...
        {SOCIALS.map((s) => (
          <a key={s.name} href={s.href} title={s.name} target="_blank" rel="noopener noreferrer"
            style={{
              ...
              border: "1px solid var(--social-border)", color: "var(--social-color)",
              textDecoration: "none", transition: "all 0.25s ease", background: "transparent",
            }}
            ...
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--social-border)"; e.currentTarget.style.color = "var(--social-color)"; e.currentTarget.style.background = "transparent"; }}
          >{s.icon}</a>
        ))}
      </div>
    </footer>
